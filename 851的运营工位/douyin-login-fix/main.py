...
def _is_douyin_qrconnect_success_status(status: str) -> bool:
    ...
    waiting_statuses = {
        "0", "1", "2", "3", "4", "5", "10",
        "new", "waiting", "pending", "scan", "scanned", "confirming",
    }
    success_statuses = {
        "confirmed", "confirm", "success", "succeeded", "done", "completed", "login_success",
    }
    ...
    if normalized.isdigit():
        return normalized not in waiting_statuses
    return False
...
async def _is_douyin_login_completed(page: Page) -> bool:
    ...
    for frame in page.frames:
        frame_state = {
            'url': frame.url,
            'has_visible_login_marker': False,
            'has_any_login_marker': False,
        }
        for kind, value in login_marker_selectors:
            try:
                if kind == 'text':
                    marker = frame.get_by_text(value, exact=True).first
                else:
                    marker = frame.get_by_role('img', name=value).first
                count = await marker.count()
                if count:
                    frame_state['has_any_login_marker'] = True
                    if await marker.is_visible():
                        frame_state['has_visible_login_marker'] = True
            except Exception:
                continue
        frame_states.append(frame_state)

    # 只要任意 frame 里登录控件仍可见，就说明登录还没真正完成
    if any(state['has_visible_login_marker'] for state in frame_states):
        return False

    # 若已经没有可见登录控件，并且至少有一个 creator 页面 frame 存在，则视为完成
    if any((state['url'] or '').startswith('https://creator.douyin.com/') for state in frame_states):
        return True
...
async def _wait_for_douyin_login(page: Page, account_file: str, qrcode_info: dict, qrcode_callback=None, poll_interval: int = 3, max_checks: int = 100) -> dict:
    ...
    seen_qrconnect_success = False
    qrconnect_events = []

    async def _on_response(resp):
        nonlocal seen_qrconnect_success
        url = resp.url.lower()
        if "check_qrconnect" not in url:
            return
        event = {"url": resp.url, "status": resp.status}
        try:
            ...
            if isinstance(data, dict):
                status = str(data.get("status") or data.get("scan_status") or data.get("result") or "")
                redirect_url = data.get("redirect_url") or data.get("redirectUrl") or data.get("url")
                if redirect_url:
                    event["redirect_url"] = redirect_url
            if status:
                event["login_status"] = status
                if _is_douyin_qrconnect_success_status(status):
                    seen_qrconnect_success = True
            if isinstance(payload, dict) and payload.get("message"):
                event["message"] = payload.get("message")
        except Exception:
            try:
                snippet = await resp.text()
                event["text"] = snippet[:500]
            except Exception:
                event["text"] = "<unreadable>"
        qrconnect_events.append(event)
        if len(qrconnect_events) > 20:
            del qrconnect_events[:-20]
        douyin_logger.info(_msg("📡", f"check_qrconnect => {event}"))
    ...
    for i in range(max_checks):
        ...
        interesting_cookies = [
            ...
            for c in cookies
            if any(key in (c.get("domain") or "") for key in ["douyin", "byted", "toutiao", "amemv", "passport"])
        ]
        ...
        if i < 3 or i % 5 == 0:
            ...
            for frame in page.frames:
                ...
                for marker_name, marker in {
                    "扫码登录": frame.get_by_text("扫码登录", exact=True).first,
                    "手机号登录": frame.get_by_text("手机号登录", exact=True).first,
                    "二维码失效": frame.get_by_text("二维码失效", exact=True).first,
                    "二维码图片": frame.get_by_role("img", name="二维码").first,
                }.items():
                    try:
                        count = await marker.count()
                        marker_states[marker_name] = {
                            "count": count,
                            "visible": await marker.is_visible() if count else False,
                        }
                    except Exception:
                        marker_states[marker_name] = {"count": -1, "visible": False}
                frame_debug.append({"url": frame.url, "markers": marker_states})
            douyin_logger.info(
                _msg(
                    "🧪",
                    f"登录轮询[{i + 1}/{max_checks}] url={page.url} title={title} cookie_count={len(interesting_cookies)} qrconnect_tail={qrconnect_events[-3:]} frames={frame_debug}",
                )
            )

        if seen_qrconnect_success:
            ...
            try:
                state_path = Path(account_file)
                state_preview = json.loads(state_path.read_text(encoding="utf-8", errors="ignore")) if state_path.exists() else {}
                saved_cookie_count = len(state_preview.get("cookies", [])) if isinstance(state_preview, dict) else -1
            except Exception:
                saved_cookie_count = -1
            douyin_logger.info(_msg("💾", f"检测到 qrconnect 成功信号，已尝试保存 storage state 到: {account_file}; runtime_cookie_count={len(interesting_cookies)} saved_cookie_count={saved_cookie_count}"))
            if saved_cookie_count > 0 or interesting_cookies:
                return _build_login_result(True, "success", "抖音扫码后已捕获 qrconnect 成功信号", account_file, qrcode_info, page.url)
    ...
    return _build_login_result(False, "timeout", f"等待抖音扫码登录超时; qrconnect_tail={qrconnect_events[-5:]}", account_file, qrcode_info, page.url)
